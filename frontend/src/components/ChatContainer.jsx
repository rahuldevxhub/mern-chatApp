  ...
  return selectedUser ? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        ...
        />
        <MoreVertIcon className="max-md:hidden max-w-5" />
      </div>
          ...
          >
            {msg.image ? (
              <img
                src={msg.image}
                alt=""
                className="max-w-25 mb-5 rounded-lg border border-gray-700"
              />
            ) : (
              <p
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3">
        <div className="flex-1 flex items-center bg-gray-100/12 px-3 rounded-full ">
          ...
          />
          <label htmlFor="image">
            <PhotoSizeSelectActualIcon className="w-5 mr-2 cursor-pointer" />
          </label>
        </div>
        <SendIcon onClick={handleSendMessage} className="w-7 cursor-pointer" />
      </div>
    </div>
  ) : (
    <div className="flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden">
      <img src={Logo} alt="logo of the chatapp" className="max-w-16" />
